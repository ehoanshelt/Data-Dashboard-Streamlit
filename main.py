...
# Filters the dataframes for visuals based on the conditions in the sidebar
...
with st.container(): # Main Content Area
    ...
    st.plotly_chart(gh.generate_lineplot(xValue = "date_created", yValue = yValue), use_container_width=True)


    st.subheader('How many orders per day?')
    st.plotly_chart(gh.generate_histplot(xValue = "total_orders"), use_container_width=True)

    st.subheader('How much revenue per day?')
    st.plotly_chart(gh.generate_histplot(xValue = "total_revenue"), use_container_width=True)
    ...
    with by_month:
        st.plotly_chart(gh.generate_boxplot("month", yValue), use_container_width=True)
    with day_of_week:
        st.plotly_chart(gh.generate_boxplot("day_of_week", yValue),use_container_width=True)
    with by_week:
        st.plotly_chart(gh.generate_boxplot("week_of_year", yValue), use_container_width=True)
